...
<%
   ...
   WHERE s.student_no = ?
   */
   int studnetNo = Integer.parseInt(request.getParameter("studentNo"));
   ...
   String sql = "SELECT s.student_no studentNo,s.student_name studentName,s.student_birth studentBirth,s.student_gender studentGender,s.createdate sCreatedate,s.updatedate sUpdatedate,s.team_no teamNo,t.team_name teamName,t.team_count teamCount,t.team_begin teamBegin,t.team_end teamEnd,t.createdate tCreatedate,t.updatedate tUpdatedate FROM student s INNER JOIN team t ON s.team_no = t.team_no WHERE s.student_no = ?";
...
<body>
   ...
   <table border="1">
      ...
      <tr>
         <th>studentGender</th>
         <td><%=(String)(map.get("studentGender"))%></td>
      </tr>
      <tr>
         <th>studentBirth</th>
	...
	<table border="1">
		<tr>
			<th>
				<button>
					<a href="<%=request.getContextPath()%>/studentList.jsp">
						학생명단으로 돌아가기	
					</a>
				</button> 	
			</th>
			<th>
				<button>
					<a href="<%=request.getContextPath()%>/updateStudentForm.jsp?studentNo=<%=(Integer)(map.get("studentNo"))%>">
						학생 정보 수정	
					</a>
				</button> 	
			</th>
			<th>
				<button>
					<a href="<%=request.getContextPath()%>/deleteStudentAction.jsp?studentNo=<%=(Integer)(map.get("studentNo"))%>">
						학생 정보 삭제	
					</a>
				</button> 	
			</th>
